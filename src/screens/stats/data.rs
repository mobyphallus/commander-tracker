...
impl Data {
    pub fn load(conn: &Connection) -> rusqlite::Result<Self> {
        let games = conn.prepare("SELECT id, started_at, win_reason, ending_turn, COALESCE(elapsed_seconds / 60.0, MAX(0, (julianday(ended_at)-julianday(started_at))*1440)) FROM games ORDER BY started_at DESC, id DESC")?.query_map([], |r| Ok(Game { id: r.get(0)?, date: r.get(1)?, reason: r.get(2)?, turns: r.get(3)?, minutes: r.get::<_, Option<f64>>(4)?.unwrap_or(0.0) }))?.collect::<rusqlite::Result<Vec<_>>>()?;
