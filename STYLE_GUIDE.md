...
- In-game identity labels use 28px semibold names on quiet dark surfaces at native size, with compact sizing and truncation only when needed to avoid controls. They use the inner edge, facing their player: player name and WUBRG mana discs (including partner colors). Keep commander names off the normal life-counter tiles; show context only when editing poison or commander damage. At compact sizes, prioritize the player name and omit secondary label details if they cannot fit clear of the timer, life counter, and action buttons. Outer-edge action cards share a consistent size and icon/title/subtitle hierarchy.
- The board center is an opaque circular control with a dark separating rim. Game time leads in stable-width digits, with turn time and menu/pause status below. Commander damage uses the same disc, with an explicit heading above Done. Outcome prompts use bounded two-column choice cards with fixed back/confirm footers.
...
- Bracket and salt scores pair a tier-stack icon and a salt-shaker icon with their numeric values and labels. Keep both icons consistent between deck tiles and the analysis screen. They are separate touch targets at the bottom-left of commander art: bracket opens the linked deck list; salt opens the score breakdown. Mana symbols sit below the name on deck tiles. The selected commander screen also offers a separate Deck list action.
